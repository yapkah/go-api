...
func TblqBonusMatchingFn(arrCond []WhereCondFn, selectColumn string, debug bool) ([]*TblqBonusMatching, error) {
	var result []*TblqBonusMatching
	tx := db.Table("tblq_bonus_matching")
	if selectColumn != "" {
...
func GetMatchingBonusByMemberId(mem_id int, dateFrom string, dateTo string) ([]*TblqBonusMatchingResult, error) {
	var (
		rwd []*TblqBonusMatchingResult
	)

	query := db.Table("tblq_bonus_matching as a").
		...
		Joins("JOIN tbl_bonus as bonus ON a.bns_id = bonus.t_bns_id AND a.member_id = bonus.t_member_id").
