...
func GetTblMMBonusSponsorFn(arrCond []WhereCondFn, selectColumn string, debug bool) ([]*TblMMBonusSponsor, error) {
	var result []*TblMMBonusSponsor
	tx := db.Table("tbl_mm_bonus_sponsor")
	if selectColumn != "" {
...
func GetMMSponsorBonusByMemberId(mem_id int, dateFrom string, dateTo string) ([]*TblMMBonusSponsorResult, error) {
	var (
		rwd []*TblMMBonusSponsorResult
	)

	query := db.Table("tbl_mm_bonus_sponsor as a").
		Select("a.t_bns_id, b.nick_name as username, down.nick_name as downline_id, a.i_lvl, a.i_lvl_paid, a.f_bv, a.f_perc, a.f_bns, a.f_bns_burned, a.dt_created").
